...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i /root/key02.pem kube-scheduler*.pem root@${node_ip}:/etc/kubernetes/cert/
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    ...
    scp -i /root/key02.pem kube-scheduler-${node_ip}.kubeconfig root@${node_ip}:/etc/kubernetes/kube-scheduler.kubeconfig
  done
```
...
EOF
```
after kube v1.19 use below v1beta1
``` bash
cat >kube-scheduler.yaml.template <<EOF
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i /root/key02.pem kube-scheduler-${node_ip}.yaml root@${node_ip}:/etc/kubernetes/kube-scheduler.yaml
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i /root/key02.pem kube-scheduler-${node_ip}.service root@${node_ip}:/etc/systemd/system/kube-scheduler.service
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -i /root/key02.pem root@${node_ip} "mkdir -p ${K8S_DIR}/kube-scheduler"
    ssh -i /root/key02.pem root@${node_ip} "systemctl daemon-reload && systemctl enable kube-scheduler && systemctl restart kube-scheduler"
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -i /root/key02.pem root@${node_ip} "systemctl status kube-scheduler|grep Active"
  done
```
...
tcp        0      0 10.2.0.6:10259          0.0.0.0:*               LISTEN      24406/kube-schedule
tcp        0      0 10.2.0.6:10251          0.0.0.0:*               LISTEN      24406/kube-schedule
```

``` bash
$ curl -s http://10.2.0.6:10251/metrics |head
