...
## 下载和分发 kubectl 二进制文件 https://kubernetes.io/docs/tasks/tools/install-kubectl-linux/

``` bash
cd /opt/k8s/work
# wget https://dl.k8s.io/v1.16.6/kubernetes-client-linux-amd64.tar.gz # 自行解决翻墙下载问题
# download latest stable version, v1.20.5 for 2021 Mar 19
curl -LO "https://dl.k8s.io/release/$(curl -L -s https://dl.k8s.io/release/stable.txt)/bin/linux/amd64/kubectl"
# Download the kubectl checksum file
curl -LO "https://dl.k8s.io/$(curl -L -s https://dl.k8s.io/release/stable.txt)/bin/linux/amd64/kubectl.sha256"
echo "$(<kubectl.sha256) kubectl" | sha256sum --check

root@joepoc-k8s-master01:/opt/k8s/work# install -o root -g root -m 0755 kubectl /usr/local/bin/kubectl
root@joepoc-k8s-master01:/opt/k8s/work# kubectl version --client
...
kubectl config set-cluster kubernetes \
  ...
  --embed-certs=true \
  --server=https://${NODE_ALL_IPS[0]}:6443 \
...
for node_ip in ${NODE_ALL_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -i /root/key02.pem root@${node_ip} "mkdir -p ~/.kube"
    scp -i /root/key02.pem kubectl.kubeconfig root@${node_ip}:~/.kube/config
  done
```