...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i /root/key02.pem kube-controller-manager*.pem root@${node_ip}:/etc/kubernetes/cert/
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    ...
    scp -i /root/key02.pem kube-controller-manager-${node_ip}.kubeconfig root@${node_ip}:/etc/kubernetes/kube-controller-manager.kubeconfig
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i /root/key02.pem kube-controller-manager-${node_ip}.service root@${node_ip}:/etc/systemd/system/kube-controller-manager.service
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -i /root/key02.pem root@${node_ip} "mkdir -p ${K8S_DIR}/kube-controller-manager"
    ssh -i /root/key02.pem root@${node_ip} "systemctl daemon-reload && systemctl enable kube-controller-manager && systemctl restart kube-controller-manager"
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -i /root/key02.pem root@${node_ip} "systemctl status kube-controller-manager|grep Active"
  done
```
...
tcp        0      0 10.2.0.6:10252    0.0.0.0:*               LISTEN      108977/kube-control
```
...
``` bash
$ curl -s --cacert /opt/k8s/work/ca.pem --cert /opt/k8s/work/admin.pem --key /opt/k8s/work/admin-key.pem https://10.2.0.6:10252/metrics |head
