...
cd /opt/k8s/work
source /opt/k8s/bin/environment.sh
for node_name in ${NODE_WORKER_NAMES[@]}
  do
    echo ">>> ${node_name}"
...
for (( i=0; i < 3; i++ ))
  do 
    echo ">>> ${NODE_WORKER_NAMES[i]}"
    sed -e "s/##NODE_NAME##/${NODE_WORKER_NAMES[i]}/" -e "s/##NODE_IP##/${NODE_WORKER_IPS[i]}/" kube-proxy-config.yaml.template > kube-proxy-config-${NODE_WORKER_NAMES[i]}.yaml.template
    scp kube-proxy-config-${NODE_WORKER_NAMES[i]}.yaml.template root@${NODE_WORKER_NAMES[i]}:/etc/kubernetes/kube-proxy-config.yaml
  done
```
