...
到 etcd 的 [release 页面](https://github.com/coreos/etcd/releases) 下载最新版本的发布包：l

``` bash ref:https://github.com/etcd-io/etcd/releases
cd /opt/k8s/work
wget https://github.com/etcd-io/etcd/releases/download/v3.4.14/etcd-v3.4.14-linux-amd64.tar.gz
tar -xvf etcd-v3.4.14linux-amd64.tar.gz
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i /root/key02.pem etcd-v3.4.14-linux-amd64/etcd* root@${node_ip}:/opt/k8s/bin
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -i /root/key02.pem root@${node_ip} "mkdir -p /etc/etcd/cert"
    scp -i /root/key02.pem etcd*.pem root@${node_ip}:/etc/etcd/cert/
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    scp -i /root/key02.pem etcd-${node_ip}.service root@${node_ip}:/etc/systemd/system/etcd.service
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -i /root/key02.pem root@${node_ip} "mkdir -p ${ETCD_DATA_DIR} ${ETCD_WAL_DIR}"
    ssh -i /root/key02.pem root@${node_ip} "systemctl daemon-reload && systemctl enable etcd && systemctl restart etcd " &
  done
```
...
for node_ip in ${NODE_IPS[@]}
  do
    echo ">>> ${node_ip}"
    ssh -i /root/key02.pem root@${node_ip} "systemctl status etcd|grep Active"
  done
```
